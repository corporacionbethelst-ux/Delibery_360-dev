...
export default function RiderProductivityPage() {
  ...
  // Obtenemos el primer elemento del array de seguridad (o null si está vacío)
  const riderData = riderProductivity && riderProductivity.length > 0 ? riderProductivity[0] : null;
  ...
  // Valores con fallback a 0 para evitar errores de undefined
  const completedDeliveries = riderData?.completedDeliveries || 0;
  const avgDeliveryTime = riderData?.averageDeliveryTime || 0;
  const rating = riderData?.customerRating || 0;
  // CORRECCIÓN: onTimeRate no existe, usamos onTimePercentage o slaCompliance
  const onTimePercentage = riderData?.onTimePercentage || riderData?.slaCompliance || 0;
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className="text-sm text-gray-500">{completedDeliveries}/50 entregas</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2.5">
                <div 
                  className="bg-blue-600 h-2.5 rounded-full" 
                  style={{ width: `${Math.min((completedDeliveries / 50) * 100, 100)}%` }}
                ></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className="text-sm text-gray-500">{onTimePercentage.toFixed(1)}% objetivo: 95%</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2.5">
                <div 
        ...
        <CardContent>
          <div className="space-y-3">
            <div className="flex justify-between py-2 border-b">
              ...
              <span className="font-medium">{riderData?.totalDistance ? riderData.totalDistance.toFixed(1) : 0} km</span>
            </div>
            <div className="flex justify-between py-2 border-b">
            ...
            <div className="flex justify-between py-2 border-b">
              ...
              <span className="font-medium">{riderData?.ordersPerHour ? riderData.ordersPerHour.toFixed(1) : 0}</span>
            </div>
            <div className="flex justify-between py-2 border-b">
              <span className="text-gray-600">Propinas Recibidas</span>
              <span className="font-medium">R$ {riderData?.tips ? riderData.tips.toFixed(2) : '0.00'}</span>
            </div>
            <div className="flex justify-between py-2 border-b">
            ...
            <div className="flex justify-between py-2">
              ...
              <span className="font-medium text-green-600">{riderData?.totalRatings ? ((riderData.totalRatings > 0 ? (riderData.customerRating * 20) : 0).toFixed(0)) : 0}%</span>
            </div>
          </div>
        </CardContent>
      </Card>
