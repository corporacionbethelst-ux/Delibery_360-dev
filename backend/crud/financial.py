...
from app.models.financial import FinancialTransaction, TransactionType, PaymentRule, Liquidation
...
class CRUDFinancial:
    ...
    async def get_liquidation(self, db: AsyncSession, liquidation_id: int) -> Optional[Liquidation]:
        result = await db.execute(select(Liquidation).where(Liquidation.id == liquidation_id))
        return result.scalar_one_or_none()
    ...
    async def get_liquidations_by_rider(
        ...
        result = await db.execute(
            select(Liquidation)
            .where(Liquidation.rider_id == rider_id)
            .order_by(Liquidation.period_end.desc())
            .offset(skip)
            .limit(limit)
        )
        return result.scalars().all()
